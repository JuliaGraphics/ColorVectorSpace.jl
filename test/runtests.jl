...
@testset "Colortypes" begin
    ...
    @testset "Arithmetic with Gray" begin
        ...
        @test eps(Gray{U8}) == Gray(eps(U8))  # #282
        ...
        @test typeof(acu.+acf) == Vector{Gray{Float32}}
        @test typeof(acu.-acf) == Vector{Gray{Float32}}
        @test typeof(acu+cf) == Vector{Gray{U8}}
        @test typeof(acu-cf) == Vector{Gray{U8}}
        @test typeof(acu.+cf) == Vector{Gray{U8}}
        @test typeof(acu.-cf) == Vector{Gray{U8}}
        ...
        @test typeof(acu/2) == Vector{Gray{typeof(U8(0.5)/2)}}
        @test typeof(acf.^2) == Vector{Gray{Float32}}
        @test (acu/Gray{U8}(0.5))[1] == gray(acu[1])/U8(0.5)
    ...
    @testset "Arithmetic with GrayA" begin
        ...
        @test sum(a) == GrayA(u8sum(0.8,0.5), u8sum(0.7,0.2))
        @test isapprox(a, a)
    ...
    @testset "dotc" begin
        ...
        @test dotc(U8(0.2), U8(0.3)) == Float32(U8(0.2))*Float32(U8(0.3))
        @test dotc(Gray{U8}(0.2), Gray24(0.3)) == Float32(U8(0.2))*Float32(U8(0.3))
